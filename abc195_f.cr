...
divs = [] of Array({Int64, Int64})
a.upto(b) do |i|
  div = AtCoder::Prime.prime_division(i)
  div.each do |(pr, ex)|
    cnt[pr] += 1
  end
  divs << div
...
vs = cnt.to_a.select {|(a, c)| c >= 2}.map {|(a, c)| a}
vs_set = vs.to_set
dp = Array(Int64).new(1_i64 << vs.size, 0_i64)
dp[0] = 1_i64
...
divs.each do |div|
  ...
  dp.each_with_index do |n, dp_bits|
    if (dp_bits & bits) == 0
      new_dp[dp_bits | bits] += n
    end
  end
  dp = new_dp
end
