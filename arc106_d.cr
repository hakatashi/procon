...
cache = Array(Mint).new(n, Mint.new(1_i64))
dp = Array(Mint).new(301_i64) do |i|
  ret = cache.sum
  n.times do |i|
    cache[i] *= ais[i]
  end
...
inv = Array(Mint).new(1000_i64) {|i| i == 0_i64 ? Mint.new(0_i64) : Mint.new(i.to_i64).inv}
pow2 = Mint.new(1_i64)
1_i64.upto(k) do |i|
  ans = Mint.new(0_i64)
  combination = Mint.new(1_i64)
  0_i64.upto(i) do |j|
    ans += combination * dp[i - j] * dp[j]
    combination *= (i - j) * inv[j + 1]
  end
  pow2 *= 2
  ans -= dp[i] * pow2
  ans //= 2
