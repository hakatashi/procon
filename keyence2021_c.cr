...
chars = Hash(Tuple(Int64, Int64), Char).new
k.times do
  h2r, w2r, cr = read_line.split
  h2 = h2r.to_i64
  w2 = w2r.to_i64
  c = cr[0]
  chars[{w2 - 1, h2 - 1}] = c
...
dp = {
  Array(Array(Mint)).new(h) { Array(Mint).new(w, Mint.zero) },
  Array(Array(Mint)).new(h) { Array(Mint).new(w, Mint.zero) },
  Array(Array(Mint)).new(h) { Array(Mint).new(w, Mint.zero) },
}
...
h.times do |y|
  w.times do |x|
    if x == 0 && y == 0
      if chars.has_key?({0_i64, 0_i64})
        dp[idx(chars[{0_i64, 0_i64}])][0][0] = Mint.one * 3
      else
        dp[0][0][0] = Mint.one
        dp[1][0][0] = Mint.one
        dp[2][0][0] = Mint.one
    ...
    if x != 0
      a = dp[0][y][x - 1] + dp[2][y][x - 1]
      if chars.has_key?({x, y})
        dp[idx(chars[{x, y}])][y][x] += a * 3
      else
        dp[0][y][x] += a
        dp[1][y][x] += a
        dp[2][y][x] += a
...
ans = Mint.zero
ans += dp[0][h - 1][w - 1] * (Mint.new(3_i64) ** (h * w - h - w + 1 - k))
ans += dp[1][h - 1][w - 1] * (Mint.new(3_i64) ** (h * w - h - w + 1 - k))
ans += dp[2][h - 1][w - 1] * (Mint.new(3_i64) ** (h * w - h - w + 1 - k))
p ans
