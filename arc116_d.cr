  ...
  macro static_modint(name, modulo)
    module AtCoder
      ...
      record {{name}}, value : Int64 do
        ...
        @@factorial_invs = Array(self).new(100_000_i64)
        ...
        def self.factorial_inv(n)
          if @@factorial_invs.empty?
            @@factorial_invs << self.new(1_i64)
          end
          @@factorial_invs.size.upto(n) do |i|
            @@factorial_invs << @@factorial_invs.last * Mint.new(i.to_i64).inv
          end
          @@factorial_invs[n]
        end
        ...
        def self.permutation(n, k)
          ...
          factorial(n) * factorial_inv(n - k)
        end

        def self.combination(n, k)
          raise ArgumentError.new("k cannot be greater than n") unless n >= k
          permutation(n, k) * @@factorial_invs[k]
        end
...
max_bits = (m // 2).to_s(2).size.to_i64
dp = Array(Array(Mint)).new(max_bits + 1) { Array(Mint).new(m + 1, Mint.zero) }

dp[0][0] = Mint.new(1_i64)
1_i64.upto(max_bits) do |bits|
  size = 1_i64 << bits
  (m + 1).times do |i|
    0_i64.step(by: 2, to: n) do |j|
      if i - j * (size // 2) >= 0
        dp[bits][i] += dp[bits - 1][i - j * (size // 2)] * Mint.combination(n, j)
      else
        break
      end
    end
