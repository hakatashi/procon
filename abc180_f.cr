...
class Dfs
  getter cache : Hash(Tuple(Int64, Int64, Int64), Mint)

  def initialize
    @cache = Hash(Tuple(Int64, Int64, Int64), Mint).new
  end

  def dfs(remains : Int64, remain_edges : Int64, take : Int64)
    if take == 1
      if remain_edges == 0
        return Mint.new(1_i64)
      else
        return Mint.new(0_i64)
      end
    end
    ...
    cache_key = {remains, remain_edges, take}
    ...
    1_i64.upto(remains // take) do |i|
      m = take <= 3 ? Mint.new(1_i64) : (Mint.factorial(take) // take // 2)

      ret = Mint.factorial(remains)
      ret //= Mint.factorial(take) ** i
      ret //= Mint.factorial(remains - take * i)
      ret //= Mint.factorial(i)
      ret *= m ** i

      0_i64.upto(i) do |twos|
        edges = i * take - twos
        if remain_edges >= edges
          1_i64.upto(take - 1) do |new_take|
            res += ret * dfs(remains - i * take, remain_edges - edges, new_take)
          end
        end
