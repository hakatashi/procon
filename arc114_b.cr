...
n = read_line.to_i64
fs = read_line.split.map {|n| n.to_i64 - 1}

visited = Array(Bool).new(n, false)
cnt = 0_i64
n.times do |i|
  next if visited[i]

  visited[i] = true
  in_loop = Set(Int64).new
  in_loop << i
  looped = true
  f = fs[i]
  if f != i
    until in_loop.includes?(f)
      if visited[f] && !in_loop.includes?(f)
        looped = false
        break
      end
      visited[f] = true
